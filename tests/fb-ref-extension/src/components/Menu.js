...
const Menu = (props) => {
    ...
    const updatePlaceholder = (plc, column) => {
    ...
    console.log(placeholders, 444)

    let ColumnInputList = Object.keys(placeholders).map((parameter, index) => {
        return (
            <ColumnInput key={index} name={parameter} tables={props.tables} updatePlaceholder={updatePlaceholder} />
        )
    })
